...
bool IsValidPlainScalar(
  ...
  static const RegEx& disallowed_flow = Exp::EndScalarInFlow() |
      (Exp::BlankOrBreak() + Exp::Comment()) | Exp::NotPrintable() |
...
void WriteDoubleQuoteEscapeSequence(
    ostream_wrapper& out,
    int codePoint,
  ...
  if (codePoint < 0xFF && stringEscapingStyle != StringEscaping::JSON) {
    ...
    WriteDoubleQuoteEscapeSequence(
        out, surrogatePair.first, stringEscapingStyle);
    WriteDoubleQuoteEscapeSequence(
        out, surrogatePair.second, stringEscapingStyle);
    return;
  }
      ...
      default:
        if (codePoint < 0x20 ||
          ...
          WriteDoubleQuoteEscapeSequence(out, codePoint, stringEscaping);
            ...
            stringEscaping == StringEscaping::NonAscii && codePoint > 0x7E) {
          WriteDoubleQuoteEscapeSequence(out, codePoint, stringEscaping);
        } else {
          WriteCodePoint(out, codePoint);
        }
...
bool WriteBinary(ostream_wrapper& out, const Binary& binary) {
  WriteDoubleQuotedString(
      out, EncodeBase64(binary.data(), binary.size()), StringEscaping::None);
  return true;
}
} // namespace Utils
} // namespace YAML
