...
Emitter& Emitter::Write(const std::string& str) {
  ...
  switch (strFormat) {
    ...
    case StringFormat::Literal:
      Utils::WriteLiteralString(
          m_stream, str, m_pState->CurIndent() + m_pState->GetIndent());
      break;
  }
...
const char* Emitter::ComputeFullBoolName(bool b) const {
  const EMITTER_MANIP mainFmt =
      (m_pState->GetBoolLengthFormat() == ShortBool
           ? YesNoBool
...
Emitter& Emitter::Write(char ch) {
  ...
  PrepareNode(EmitterNodeType::Scalar);
  Utils::WriteChar(
      m_stream, ch, GetStringEscapingStyle(m_pState->GetOutputCharset()));
  StartedScalar();
...
Emitter& Emitter::Write(const _Comment& comment) {
  ...
  Utils::WriteComment(
      m_stream, comment.content, m_pState->GetPostCommentIndent());
