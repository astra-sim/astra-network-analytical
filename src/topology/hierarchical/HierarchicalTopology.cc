...
HierarchicalTopology::HierarchicalTopology(TopologyConfigs configs, HierarchicalTopologyConfig hierarchy_config, CostModel& cost_model) noexcept :
...
HierarchicalTopology::Latency HierarchicalTopology::linkLatency(int dimension, int hops_count) const noexcept {
    return hops_count * configs[dimension].getLinkLatency();
}

double HierarchicalTopology::send(NpuId src, NpuId dest, PayloadSize payload_size) noexcept {
    checkNpuIdBound(src);
...
HierarchicalTopology::NpuAddress HierarchicalTopology::npuIdToAddress(NpuId npu_id) const noexcept {
...
HierarchicalTopology::NpuId HierarchicalTopology::npuAddressToId(NpuAddress npu_address) const noexcept {
    // If units-count if [2, 8, 4], and the address is [1, 7, 3], then the id should be
    // address * (topology size up to the dimension)
    //       1 * (1)
    //     + 7 * (1 * 2)
    //     + 3 * (1 * 2 * 8)
    auto id = 0;
