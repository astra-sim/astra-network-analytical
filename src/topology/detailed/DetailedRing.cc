...
double DetailedRing::send(NpuId src, NpuId dest, PayloadSize payload_size) noexcept {
...
DetailedRing::Direction DetailedRing::computeDirection(NpuId src, NpuId dest) {
    ...
    auto reverse_distance = npus_count - natural_distance;  // 0->9->8 (src < dest, -1) or 8->9->0->1 (src > dest, +1)
...
DetailedRing::NpuId DetailedRing::takeStep(NpuId current_node, Direction direction) {
    // take a step
