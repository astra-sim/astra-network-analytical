...
DetailedTopology::DetailedTopology(
...
void DetailedTopology::connect(NpuId src, NpuId dest, int dimension) noexcept {
  ...
  assert(
      std::get<1>(create_result) &&
      "[DetailedTopology, method connect] Link already exists");
}

DetailedTopology::Latency DetailedTopology::transmit(
    NpuId src,
    NpuId dest,
  ...
  assert(
      src_entry != links.end() &&
      "[DetailedTopology, method transmit] Link doesn't exist.");

  auto dest_entry = src_entry->second.find(dest);
  assert(
      dest_entry != src_entry->second.end() &&
      "[DetailedTopology, method transmit] Link doesn't exist.");
