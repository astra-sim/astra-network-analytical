...
double FastRing_AllToAll_Switch::send(NpuId src, NpuId dest, PayloadSize payload_size) noexcept {
...
FastRing_AllToAll_Switch::NpuAddress FastRing_AllToAll_Switch::npuIdToAddress(NpuId npu_id) const noexcept {
    auto ring_size = configs[0].getNpusCount();
...
//    std::cout << npu_id << ": " << ring_id << " " << all_to_all_id << " " << node_id << std::endl;
    return npu_address;
}

FastRing_AllToAll_Switch::NpuId FastRing_AllToAll_Switch::npuAddressToId(NpuAddress npu_address) const noexcept {
    auto ring_size = configs[0].getNpusCount();
    auto all_to_all_size = configs[1].getNpusCount();
    auto node_size = ring_size * all_to_all_size;

    return npu_address[0] + (ring_size * npu_address[1]) + (node_size * npu_address[2]);
}
