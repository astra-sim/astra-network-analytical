...
double Switch::sendAndGetLatency(NodeAddress src, NodeAddress dest, int payload_size) noexcept {
    auto srcID = src[0];
    auto destID = dest[0];  // FIXME: IDs not used, use it when logging is required

    const auto hops_count = 2;  // Switch
    ...
    network_latency += configuration.getRouterLatency();  // Switch latency
