...
void get_comm_topology(
  ...
  for (int i = 0; i < dimensions_count; i++) {
    auto link_bandwidth_b_ns =
      (double)link_bandwidths[i] * (1 << 30) / (1'000'000'000); // link bandwidth in B/ns
  ...
  if (topology_name == "Hierarchical") {
    ...
    auto hierarchy_config =
      Analytical::HierarchicalTopologyConfig(
        dimensions_count,
        topologies_per_dim,
        dimension_types,
        links_count_per_dim,
    ...
    topology = make_shared<Analytical::HierarchicalTopology>(topology_configs, hierarchy_config);
  } else {
    cout << "[Analytical, main] Topology not defined: " << topology_name << endl;
  }
}
...
int main(int argc, char* argv[]) {
  try {
    ...
    options
      ...
      ("comm-scale", "Communication scale", cxxopts::value<double>()->default_value("1"))
      ("injection-scale", "Injection scale", cxxopts::value<double>()->default_value("1"))
      ("rendezvous-protocol", "Whether to enable rendezvous protocol", cxxopts::value<bool>()->default_value("false"))
    ;
    ...
    auto physical_dims = vector<int>(); // number of nodes for each system layer dimension
